...
internal static class TarballUtilities
{
    ...
    public static string SanitizePackagePath(string packageName) => packageName.Replace('/', '_').Replace('\\', '_');
    ...
    private static string? NormalizeTarEntryName(string entryName)
    {
        ...
        {
            normalized = normalized["package/".Length..];
        }
    ...
    private static string ReadString(byte[] buffer, int offset, int count)
    {
        ...
        return (terminator >= 0 ? value[..terminator] : value).Trim();
    }
